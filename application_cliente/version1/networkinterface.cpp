...
void NetworkInterface::receiveMessageAction(QByteArray *message)
{
	...
	if(arg->value("action")=="MODIFIED")
	{
		...
		if(realPath.endsWith("/")){emit receiveErrorMessage("1");return;}
	...
	else if(arg->value("action")=="CREATED")
	{
		...
		if(realPath.endsWith("/")) realPath2=realPath.left(realPath.length()-1);
		Dir *parent=(Dir*)configurationData->getConfigurationFile()->findMediaByRealPath(realPath2.left(realPath2.lastIndexOf("/")));
		if(parent==NULL){emit receiveErrorMessage(realPath2.left(realPath2.lastIndexOf("/")));return;}
...
bool NetworkInterface::sendFileModified(File *f,QByteArray content)
{
	if(!socket->isWritable()) return false;
...
bool NetworkInterface::sendMediaCreated(Media *m)
{
	if(!socket->isWritable()) return false;
...
//supprimé
bool NetworkInterface::sendMediaRemoved(Media *m)
{
	if(!socket->isWritable()) return false;
	QString realPath=m->getRealPath();
	if(realPath=="") return false;
